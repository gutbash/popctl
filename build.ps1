# Build popctl.asi (32-bit) with the VS2022 x86 toolchain; -Deploy copies the .asi + .ini to the game root.
[CmdletBinding()]
param([switch]$Deploy)
...
$src = Join-Path $here 'popctl.cpp'
$dll = Join-Path $obj  'popctl.dll'
$asi = Join-Path $here 'popctl.asi'
$cmd = @"
...
if ($Deploy) {
    ...
    Copy-Item $asi (Join-Path $game 'popctl.asi') -Force
    if (-not (Test-Path (Join-Path $game 'popctl.ini'))) { Copy-Item (Join-Path $here 'popctl.ini') (Join-Path $game 'popctl.ini') }
